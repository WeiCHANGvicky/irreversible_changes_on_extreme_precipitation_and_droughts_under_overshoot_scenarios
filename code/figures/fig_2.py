# -*- coding: utf-8 -*-
"""
Created on Mon Aug 24 21:12:32 2026
...
# 4. 数据处理逻辑
# =============================================================================
def process_scenario_data(pr_path, tas_path, p1_span, p4_span, p2_span, target_temp):
    ...
    for pf in pr_files:
        ...
        pr = ds_p['r95ptot'].where((ds_p.lat >= -60) & (ds_p.lat <= 90), drop=True)
...
data_126 = process_scenario_data(
    pr_path=r'E:\data\etccdi\ssp126\r95ptot\*.nc',
    tas_path=r'E:\data\tas\annual\ssp126\*.nc',
    p1_span=(2035, 2055), p2_span=(2170,2190), p4_span=(2280, 2300), target_temp=1.83
)

data_534 = process_scenario_data(
    pr_path=r'E:\data\etccdi\ssp534\r95ptot\*.nc',
    tas_path=r'E:\data\tas\annual\ssp534\*.nc',
    p1_span=(2025, 2045), p2_span=(2165,2185), p4_span=(2280, 2300), target_temp=1.866
)
...
def draw_scatter_subplot(ax, gmst_ts, pr_ts, target_temp, panel_label, region_title, ks_p1, ks_p4, inset_pos=[0.58, 0.12, 0.35, 0.32], target_years=None):
    ...
    ax.set_ylabel('R95pTOT Anomaly (%)', fontsize=10, labelpad=2)
...
im_a = draw_map_subplot(ax_a, data_126['map_data'], data_126['map_agree'], '(a)', 'R95pTOT','SSP1-2.6')
im_b = draw_map_subplot(ax_b, data_534['map_data'], data_534['map_agree'], '(b)', 'R95pTOT', 'SSP5-3.4-OS')
...
# =============================================================================
# 9. 提取并控制台打印 P1-P4 对应的 R95pTOT Anomaly (%) 数值
# =============================================================================

panels_info = [
    ('(c) SSP1-2.6 Eurasia', data_126['eu_smooth'], targets_c),
    ('(e) SSP1-2.6 Amazon',  data_126['am_smooth'], targets_e),
    ('(d) SSP5-3.4 Eurasia', data_534['eu_smooth'], targets_d),
    ('(f) SSP5-3.4 Amazon',  data_534['am_smooth'], targets_f),
]

print("\n" + "=" * 65)
print(f"{'Panel / Subplot':<25} | {'Label':<6} | {'Year':<6} | {'R95pTOT Anomaly (%)':<18}")
print("-" * 65)
...
for title, ts_data, targets in panels_info:
    for target in targets:
        ...
        if yr in ts_data['year'].values:
            ...
            print(f"{title:<25} | {label:<6} | {yr:<6} | {val:+.2f}%")
        else:
            print(f"{title:<25} | {label:<6} | {yr:<6} | N/A")
    print("-" * 65)
print("\n")
...
plt.savefig('Figure02.pdf', dpi=800, bbox_inches='tight')
