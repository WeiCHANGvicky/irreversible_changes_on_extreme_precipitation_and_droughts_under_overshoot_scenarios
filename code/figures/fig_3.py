# -*- coding: utf-8 -*-
"""
Created on Mon Aug 24 21:36:09 2026
...
# 4. 数据处理逻辑
# =============================================================================
def process_scenario_data(pr_path, tas_path, p1_span, p2_span, p3_span, p4_span, target_temp):
    ...
    for pf in pr_files:
        ...
        p2 = pr.sel(year=slice(p2_span[0], p2_span[1])).mean('year')
        p3 = pr.sel(year=slice(p3_span[0], p3_span[1])).mean('year')
...
data_126 = process_scenario_data(
    pr_path=r'E:\data\spi\year\ssp126\spi6\*.nc',
    tas_path=r'E:\data\tas\annual\ssp126\*.nc',
    p1_span=(2035, 2055), p2_span=(2065,2085), p3_span=(2170,2190), p4_span=(2280, 2300), target_temp=1.83
)

data_534 = process_scenario_data(
    pr_path=r'E:\data\spi\year\ssp534\spi6\*.nc',
    tas_path=r'E:\data\tas\annual\ssp534\*.nc',
    p1_span=(2025, 2045), p2_span=(2055,2075), p3_span=(2165,2185), p4_span=(2280, 2300), target_temp=1.866
)
...
def draw_scatter_subplot(ax, gmst_ts, pr_ts, target_temp, panel_label, region_title, ks_p1, ks_p4, inset_pos=[0.58, 0.12, 0.35, 0.32], target_years=None):
    ...
    ax.set_ylabel('Annual minimum SPI-6', fontsize=10, labelpad=2)
    ax.set_title(f'{panel_label}', fontsize=10, fontweight='bold', loc='left')
...
im_a = draw_map_subplot(ax_a, data_126['map_data'], data_126['map_agree'], '(a) Annual minimum SPI-6', 'SSP1-2.6')
im_b = draw_map_subplot(ax_b, data_534['map_data'], data_534['map_agree'], '(b) Annual minimum SPI-6', 'SSP5-3.4-OS')
...
sc_c = draw_scatter_subplot(
    ...
    inset_pos=[0.45, 0.13, 0.35, 0.32], target_years=targets_c
)
...
# =============================================================================
# 9. 提取并控制台打印 P1-P4 对应的 Annual Minimum SPI-6 数值
# =============================================================================
panels_info = [
    ('(c) SSP1-2.6 North Africa', data_126['na_smooth'], targets_c),
    ('(d) SSP5-3.4 North Africa', data_534['na_smooth'], targets_d),
]

print("\n" + "=" * 65)
print(f"{'Panel / Subplot':<28} | {'Label':<6} | {'Year':<6} | {'Annual Min SPI-6':<18}")
print("-" * 65)

for title, ts_data, targets in panels_info:
    for target in targets:
        yr, label = target[0], target[1]
        if yr in ts_data['year'].values:
            val = float(ts_data.sel(year=yr).values)
            print(f"{title:<28} | {label:<6} | {yr:<6} | {val:+.4f}")
        else:
            print(f"{title:<28} | {label:<6} | {yr:<6} | N/A")
    print("-" * 65)
print("\n")
...
# 10. 保存与显示
# =============================================================================
plt.savefig('Figure03.pdf', dpi=800, bbox_inches='tight')
plt.savefig('Figure03.png', dpi=800, bbox_inches='tight')
plt.show()